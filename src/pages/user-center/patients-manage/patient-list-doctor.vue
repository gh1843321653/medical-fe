<template>
  <div class="guahao-list">
  		<form class="search"   refs="form" action="" method="post" @submit="search($event)">
			...
			<input  class="input-search" type="search" v-model="searchData.keyWord" placeholder="姓名、性别、年龄"/>
		         ...
		         <li  class="list-item" v-for="(item,index) in listData" >
		         	<router-link to="">
		         		<div class='th'>
		         			...
		         			<span>{{item.patientSex==='0'?'男':'女'}}</span>
		         			<span>{{item.patientAge}}岁</span>
	         		  </div>
	         		  <div class="tr">
	         		  	<router-link :to="{path:'/patient-view',query:{patientId:item.patientId}}">基本信息</router-link>
	         		  	<router-link :to="{path:'/appl-ecg-list-doctor',query:{guahaoId:item.guahaoId,patientId:item.patientId,patientName:item.patientName}}">心电申请</router-link>
	         		  	<router-link :to="{path:'/report-list',query:{patientId:item.patientId}}">检查报告</router-link>
	         		  </div>
		         	</router-link>
		         </li>
		       </ul>
        ...
        <group class="search-box">
          ...
          <ul class="main">
          	<li class='search-item'>
          		<h1 class='item-title'>监护状态</h1>
	      			<checker v-model="searchData.jianhuState" class=" item-checker "default-item-class="default-style" selected-item-class="selected-style">
						      <checker-item value="">全部</checker-item>
						      <checker-item v-for="item in initData.jianhuState" :value="item.code" :key="item.code">{{item.name}}</checker-item>
						      <checker-item value="0">急救</checker-item>
						       <checker-item value="1">监护</checker-item>
					     </checker>
          	</li>
          	
          	<li class='search-item'>
          		<h1 class='item-title'>监护日期</h1>
...
export default{
 ...
 data () {
    return {
      ...
      initData:[],//选项类数据
      searchData:{//查询条件
      	jianhuState:'',//监护状态(急救|监护)
      	guahaoType:'1',//门诊挂号(我的病人指挂号类型为门诊挂号的列表)
