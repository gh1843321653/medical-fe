<template>
	<form id='cp-regist' ref='cp-regist'>
		<group ref='group'>
			<x-input title="" ref='newMobile' v-model='newMobile' placeholder="请输入新手机号码"  keyboard="number" required is-type="china-newMobile" :max='11':show-clear="true"   @input="activate" >
  ...
  methods: {
    ...
    /*获取验证码*/
  	getVcode:function(){
  		...
  		this.disabled=true;
  		/*ajax请求验证码...*/
  		let submitData={userName:this.newMobile}
    ...
    submit:function(ev){
    	...
    	if(this.validator()){
			...
			ajax.changeAccount(submitData)
			.then(function(res){
				let time=1500
				the.$vux.toast.show({type:'text',text:'账号更改成功',time:time})
				the.loginOut()
				setTimeout(function(){
